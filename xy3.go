...
// or streaming content in general (i.e. io.Reader), use UploadStream or [s3manager Uploader]. The latter is smart
// enough to use a single PutObject if the content can fit in a single call. Upload and UploadStream always use
// multipart upload.
//
...
// UploadStream uploads streaming content of unknown size specified by the given io.Reader.
//
// Unlike [s3manager Uploader] which is smart enough to use a single PutObject if the content fits in a single call,
// UploadStream always uses multipart upload.
//
// If you would like to use [s3manager Uploader] but want to add progress report, check out
// managerlogging.WrapUploadAPIClient which provides progress logging by decorating the manager.UploadAPIClient
// and manager.DownloadAPIClient instance.
//
// [s3manager Uploader]: https://pkg.go.dev/github.com/aws/aws-sdk-go-v2/feature/s3/manager
func UploadStream(ctx context.Context, client UploadAPIClient, body io.Reader, input *s3.CreateMultipartUploadInput, optFns ...func(*UploadOptions)) (*s3.CompleteMultipartUploadOutput, error) {
	u, err := newMultipartUploader(client, optFns...)
	if err != nil {
		return nil, err
	}

	return u.uploadStream(ctx, body, input, 0)
}
